...
| **Live commit (console)** | **`aa8376a`** (footer / `/api/version`) · **API live `6f48153`** (heartbeat carries `Enforce`; staged `aa8376a`) · **agent `aa8376a`** (after .181 redeploy). Reliable stamp = footer = git HEAD |
...
| **Agent (test) .181** | **PILOT SUCCESSFUL** – `.181` = **TRNKAMW11** (own workstation); service "USB Guardian" RUNNING, heartbeat + **incidents flowing into DB**. Agent live **`aa8376a`** – user attribution, client 100% (watchdog+toast), **enforcement P1-3 + auto-re-enable**. Updating the agent needs elevation (UAC) → run by the user (build staged on .213) |
