...
Scheduled task on the app server (as gmsa-USBGdep$) → Deploy-AgentFleet.ps1 -TargetsFile … → installation
```
...
New-ADGroup -Name "USB-Guardian-Deployers" -GroupScope Global `
    -Path "OU=Service Accounts,DC=axinetwork,DC=loc"   # adjust the OU
```

### 2. (DC) gMSA + allow the app server to retrieve the password + add to the group

```powershell
# KDS root key – once per domain (you most likely already have one because of gmsa-SQL$):
...
New-ADServiceAccount -Name "gmsa-USBGdep" `
    ...
    -PrincipalsAllowedToRetrieveManagedPassword "B-S-W-MIKOS$"   # the app server's machine account
...
Test-ADServiceAccount  gmsa-USBGdep      # must return True
```
...
## The second deploy account: `gmsa-USBGsrv$` (API deployment)
...
**console service would have to run as `gmsa-USBGdep$`** – but then that account also needs the **same SQL
