# ============================================================
# Install-Agent.ps1
# Instalace USB Guardian agenta jako Windows sluzby (SYSTEM).
#
# Co dela:
#   1. Zkopiruje self-contained publish do C:\Program Files\USBGuardian
#      (ZACHOVA existujici Config\agent.config.local.json - per-machine
#       sync nastaveni se NEPREPISE).
#   2. Vytvori/aktualizuje sluzbu "USB Guardian" (start=auto, LocalSystem)
#      + recovery (restart pri padu).
#   3. Zaregistruje watchdog scheduled task (Register-Watchdog.ps1).
#   4. Spusti sluzbu.
#
# Pouziti (z publish vystupu):
...
#
# Pozn.: agent potrebuje Config\agent.config.local.json se syncUrl +
#   tls.pinnedThumbprint (jinak whitelist sync nepojede). Skript na to upozorni.
#
# Skript si sam vyzada UAC elevaci.
# ============================================================
...
# -- 1) Zastavit existujici sluzbu (kvuli zamceni souboru) --
